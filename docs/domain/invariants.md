...
- Service: `VALID_STATUS_TRANSITIONS.SOLD = []` in `listingsService.update`; `expireReservations` `WHERE status = 'RESERVED'`; `paymentsService.confirmPayment` sells only `RESERVED` + matching `reservedByOrderId`. `listingsService.cancel` guards the transition with a conditional `tx.listing.updateMany({ where: { id, status: { not: "SOLD" } }, data: { status: "CANCELED" } })` inside the same transaction as its audit write; the pre-transaction ownership read is authorization-only and is never the correctness mechanism. A listing that has successfully committed `SOLD` can never subsequently be overwritten to `CANCELED`; if cancel's conditional update instead commits first, payment's own conditional `status = RESERVED` listing update matches nothing and that transaction (including its order claim) rolls back, so a `CANCELED` listing is never left paired with a `PAID` order either (AUD-001, PR11).
- Tests: `server/src/__tests__/reservation.lifecycle.integration.test.ts` (does not expire `SOLD`; payment vs expiry races; `AUD-001: racing cancel against a concurrent payment confirmation never produces a paid order with a canceled listing, or a sold listing whose order never reached PAID` races `paymentsService.confirmPayment` against `listingsService.cancel` on the same reservation over real PostgreSQL across independent trials, asserting both legal outcomes rather than a fixed winner); `server/src/modules/listings/__tests__/listings.reservation.test.ts`; `server/src/modules/listings/__tests__/listings.invariants.test.ts` (`SOLD` PATCH/cancel).
