...
- Service: `confirmPayment` appends the credit. `applyRefundCompensation` appends exact inverses only for existing credits and updates each balance in the same transaction; duplicate insert attempts are no-ops. Reconciliation SETs drift to applied `SUM(netAmount)` under `FOR UPDATE` and never rewrites ledger rows.
...
| `INV-LEDGER-APPEND-ONLY` | Applied credits remain visible; compensation is a separate exact signed movement and cannot double-debit balance. | entry shape CHECK + economic-event uniqueness + one DB transaction | `refund.ledger.integration.test.ts` |
