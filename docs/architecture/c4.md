# C4 — Neon Arsenal Market

This is the **current canonical** system represented by the repository's deployment configuration and code. It is not an AWS target-state drawing, and configuration alone is not evidence of live availability.

The canonical topology is a **Vercel frontend plus Render API and PostgreSQL**: `vercel.json` defines the Vite deployment, while `render.yaml` declares Docker web service `neon-arsenal-api` and managed PostgreSQL `neon-arsenal-db`. PayPal, Resend, and optional cs2.sh catalog import sit outside the process. There is **no** ECS/Fargate cluster, **no** ALB-as-live, **no** RDS, **no** Secrets Manager, **no** Terraform in this repository.

A **future** ECS/Fargate sketch exists on the P2 roadmap (`docs/roadmap.md`). It remains only an alternative unless measured need and a later human-approved ADR supersede ADR 0007. Do not read this file as if ECS were running.

**Related:** `docs/architecture/current-state.md`, `docs/architecture/sequences.md` (checkout, concurrent reserve, webhook, reconciliation), `docs/operations/runbook.md`, `docs/architecture/threat-model.md`, `docs/architecture/scaling-path.md`, `docs/architecture/capacity.md`.

---

## Level 1 — System context

People and external systems that talk to Neon Arsenal. The software system is one marketplace (Vite UI + Express API + PostgreSQL).

```text
                    ┌──────────────┐
                    │   Customer   │
                    └──────┬───────┘
                           │ HTTPS (browse, auth, checkout)
                           ▼
┌──────────┐        ┌─────────────────────┐        ┌──────────────┐
│  Seller  │───────►│  Neon Arsenal       │◄──────►│    PayPal    │
└──────────┘ HTTPS  │  Market             │ REST + │  (Orders,    │
                    │  (this repository)  │ webhook│   Capture,   │
┌──────────┐        │                     │        │   webhooks)  │
│  Admin   │───────►│                     │        └──────────────┘
└──────────┘ HTTPS  └──────────┬──────────┘
                               │ email
                               ▼
                        ┌──────────────┐
                        │    Resend    │
                        └──────────────┘
```

```mermaid
C4Context
  title System context — current
  Person(customer, "Customer", "Registers, browses unique listings, pays with PayPal")
  Person(seller, "Seller", "Lists items; sees orders that include their items; pending admin approval")
  Person(admin, "Admin", "Approves sellers; admin APIs")
  Person(operator, "Operator", "Vercel/Render dashboards: deploys, env, logs")

  System(nam, "Neon Arsenal Market", "Modular-monolith marketplace for unique CS2-style listings")

  System_Ext(paypal, "PayPal", "Create/capture orders; signed webhooks")
  System_Ext(resend, "Resend", "Verification and transactional email")
  System_Ext(cs2sh, "cs2.sh", "Optional catalog metadata and price reference import")

  Rel(customer, nam, "HTTPS JSON + Bearer JWT")
  Rel(seller, nam, "HTTPS JSON + Bearer JWT")
  Rel(admin, nam, "HTTPS JSON + Bearer JWT")
  Rel(operator, nam, "Deploy and configure")
  Rel(nam, paypal, "OrdersCreate, OrdersCapture, order GET")
  Rel(paypal, nam, "POST /payments/webhook (also /api/v1/payments/webhook)")
  Rel(nam, resend, "Send verification email")
  Rel(nam, cs2sh, "Optional catalog import")
```

| Actor | Relationship |
|---|---|
| Customer / Seller / Admin | Browser calls the public API with JSON and `Authorization: Bearer`. Roles are JWT claims plus domain ownership checks. |
| Operator | Vercel and Render dashboards. Not an application user. |
| PayPal | Unreliable external ledger. Local sale is PostgreSQL, not a client “paid” flag. |
| Resend | Email delivery. Not an authorization server. |
| cs2.sh | Optional catalog import boundary. It does not create, reprice, or revive marketplace inventory. |

---

## Level 2 — Containers (canonical deployables)

C4 “container” here means a **deployable / process**, not Docker-only.

### Canonical deployment topology

```text
Internet
  │
  ├─► Vercel Vite SPA (`vercel.json`)
  │        │  API_URL (build-time public config)
  │        ▼
  └─► Render web service `neon-arsenal-api`
           Docker: server/Dockerfile
           bind 0.0.0.0:$PORT (Blueprint PORT=3001)
           healthCheckPath: /ready
           │
           ├─► Render Postgres `neon-arsenal-db`   (source of truth)
           ├─► PayPal REST + inbound webhook
           ├─► Resend
           └─► cs2.sh (optional catalog import)
```

```mermaid
C4Container
  title Containers — canonical (Vercel frontend + Render API/Postgres)
  Person(user, "Browser user")

  Container_Ext(spa, "Vite SPA", "React on Vercel", "Static assets and SPA rewrite from vercel.json.")
  Container(api, "Express API", "Node / Docker on Render", "Modular monolith. HTTP, JWT, domain modules, in-process jobs.")
  ContainerDb(db, "PostgreSQL", "Render managed", "Listings, orders, payments, users. Prisma.")
  System_Ext(paypal, "PayPal")
  System_Ext(resend, "Resend")
  System_Ext(cs2sh, "cs2.sh")

  Rel(user, spa, "HTTPS")
  Rel(spa, api, "HTTPS JSON, Bearer, Idempotency-Key")
  Rel(user, api, "HTTPS (curl, webhooks are not the SPA)")
  Rel(api, db, "Prisma")
  Rel(api, paypal, "Orders create/capture/GET")
  Rel(paypal, api, "Webhook POST /payments/webhook or /api/v1/payments/webhook")
  Rel(api, resend, "Email API")
  Rel(api, cs2sh, "Optional catalog import")
```

| Container | Where it lives | Notes from the repo |
|---|---|---|
| Vite SPA | **Vercel** (`vercel.json`, `README.md`) | Not a security boundary. Needs `API_URL` pointing at the public API origin. SPA rewrite to `index.html`. |
| Express API | Render `neon-arsenal-api`, Docker, context `server/` | Entrypoint: migrate → optional seed → `node dist/index.js`. `GET /health` liveness (Docker HEALTHCHECK). `GET /ready` readiness (Render probe). |
| PostgreSQL | Render `neon-arsenal-db` | Authoritative business state. No Redis, no queue, no replica claimed as live. |
| PayPal | External | Sandbox in the Blueprint (`PAYPAL_MODE=sandbox`). Credentials `sync: false`. |
| Resend | External | `RESEND_API_KEY`, `EMAIL_FROM` `sync: false`. |
| cs2.sh | External, optional | `CS2SH_API_KEY` is `sync: false`; import failure does not block readiness. |

Secrets are **Render environment variables** (`JWT_*` `generateValue`; PayPal/Resend `sync: false`). That is not AWS Secrets Manager.

### What is inside the API process (not extra containers)

One Node process does HTTP **and** timers. Adding replicas duplicates those timers; they are written to be idempotent against PostgreSQL (`docs/architecture/scaling-path.md`).

| In-process job | Interval (as documented) | Role |
|---|---|---|
| Reservation expiry | 30s | Conditional `RESERVED → ACTIVE` when TTL elapsed; cannot overwrite `SOLD`. |
| PayPal GET reconciliation | 60s, min age 2 min, batch 20 | Recover COMPLETED captures if webhook was lost; OrdersCapture live APPROVED holds. Cannot sell an expired hold. |
| Seller ledger reconciliation | 60s | Compare `Seller.balance` to PAID `SUM(netAmount)`; SET projection if they disagree (ledger wins). |

OpenTelemetry is **optional** (`OTEL_ENABLED` defaults off). There is no sidecar collector in `render.yaml`.

### Not live (do not draw as current)

```text
Internet → ALB → ECS/Fargate → RDS → Secrets Manager → CloudWatch
```

That box is P2 **target-architecture prose** on the roadmap. It is **not** provisioned. `ADR 0007` (P-back activity **C1**) already decided it stays a future option, not a committed migration; only a later human-approved ADR that supersedes ADR 0007 changes that.

Also not present as containers: Redis, Kafka, RabbitMQ, SQS, a separate worker service, API gateway product.

---

## Level 3 — Components (inside the Express API)

Preferred dependency direction (same as `current-state.md`):

```text
Routes / Controllers  →  Application / Domain services  →  Repositories  →  Prisma / PostgreSQL
```

Some services still call Prisma directly; that is a known inconsistency, not a second architecture.

```text
                         ┌─ requestId, security headers, CORS, JSON+rawBody 100kb, rate limit
 Incoming HTTP ──────────┤
                         └─ /health  /ready  /docs
                                    │
        ┌──────────┬──────────┬─────┴──────┬──────────┐
        │ auth     │ users    │ sellers    │ products (Catalog) │
        │ listings │ orders   │ payments   │ commissions (Ledger)│
        │ reviews  │ favorites│ admin      │ audit              │
        └──────────┴──────────┴────────────┴──────────┘
                                    │
                    shared: jwt, hash, paypal client,
                    webhook verify, errors, observability,
                    shutdown, jobs
                                    │
                              Prisma ──► PostgreSQL
```

```mermaid
C4Component
  title API components — modular monolith (one deployable)
  Container_Boundary(api, "Express API process") {
    Component(http, "HTTP edge", "app.ts", "security headers, CORS, JSON 100kb+rawBody, apiLimiter, errorHandler")
    Component(auth, "auth", "modules/auth", "Register, verify email, login, refresh, logout")
    Component(listings, "listings", "modules/listings", "Unique items; reserve/expire")
    Component(orders, "orders", "modules/orders", "Checkout + Idempotency-Key")
    Component(payments, "payments", "modules/payments", "PaymentLink, webhook, confirmPayment")
    Component(other, "other domains", "users, sellers, products/Catalog, commissions/Ledger, reviews, favorites, admin, audit")
    Component(jobs, "in-process jobs", "shared/jobs", "Expiry + PayPal GET + seller ledger reconcile + outbox")
    Component(paypalc, "Outbound PayPal gateway", "modules/payments", "Create/capture/GET/refund; delegates to shared provider utilities")
    ComponentDb(db, "PostgreSQL", "Prisma")
  }
  System_Ext(paypal, "PayPal")
  Rel(http, auth, "routes")
  Rel(http, listings, "routes")
  Rel(http, orders, "routes")
  Rel(http, payments, "routes")
  Rel(http, other, "routes")
  Rel(orders, listings, "reserve in one transaction")
  Rel(payments, paypalc, "OrdersCreate after PaymentLink claim")
  Rel(paypal, payments, "webhook")
  Rel(jobs, listings, "expireReservations")
  Rel(jobs, payments, "reconcile PENDING")
  Rel(jobs, other, "reconcile Seller.balance")
  Rel(auth, db, "users, RefreshToken, LoginThrottle")
  Rel(listings, db, "Listing")
  Rel(orders, db, "Order, OrderIdempotencyKey")
  Rel(payments, db, "PaymentLink, PaymentWebhookEvent")
```

| Component | Responsibility that matters for interviews |
|---|---|
| HTTP edge (`app.ts`) | Untrusted input. Captures `rawBody` for webhook signatures. Global in-memory rate limit. |
| auth | bcrypt passwords; Bearer access JWT; refresh token families (`RefreshToken` allowlist, reuse revokes family); per-email login throttle. ADMIN is not a public register role. |
| listings / orders | Unique listing: conditional `ACTIVE → RESERVED` in the same transaction as the order. |
| payments | Client cannot mark paid. `PaymentLink` before `OrdersCreate`. Capture after approval (`POST /payments/capture`). The module-local outbound gateway from ADR 0025 delegates to PayPal utilities; webhook verification remains a separate inbound trust boundary. Only PayPal `COMPLETED` sells. |
| Catalog / Ledger | Catalog = `modules/products`. Ledger = `modules/commissions`. Import rules: `docs/architecture/modular-monolith.md`. |
| jobs | Same invariants as HTTP paths; they are not a second source of truth. |
| Prisma / PostgreSQL | Constraints (idempotency key, webhook event id, seller txn per order) enforce what the process might retry. |

---

## Trust and failure (C4 does not replace these)

- Trust boundaries and abuse controls: `docs/architecture/threat-model.md`.
- Crash, retry, webhook duplicate, capture-after-expiry: `docs/architecture/failure-modes.md` and `docs/operations/runbook.md`.
- Scaling: stay on this container diagram until a **measured** trigger in `docs/architecture/scaling-path.md`. Extra API replicas are more of the same container, not new services. Sequence diagrams: `docs/architecture/sequences.md`.

---

## How to use this in an interview

1. Draw **context** (people, PayPal, Resend, optional cs2.sh).
2. Draw **containers** as Vercel SPA + Render API + Render Postgres — say **ECS is not live**.
3. Zoom into **one process**: modules + in-process jobs + PostgreSQL constraints.
4. If asked “where is the queue?”, answer: there isn’t one; uniqueness and conditional updates are the coordination.
