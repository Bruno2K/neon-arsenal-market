...
## Operator failure-mode matrix

| Failure | Detection | Immediate effect | Durable state | Automatic recovery | Operator action | Evidence |
|---|---|---|---|---|---|---|
| PostgreSQL unavailable | `/ready` 503; Prisma error/log; `db.client.errors` if OTLP exports | Instance is not ready; DB-backed requests fail | Previously committed rows unchanged | Render readiness probe observes the service as unready; traffic-routing behavior depends on provider health-check semantics; app recovers when DB returns | Check Render DB/service health and connection configuration; do not use `/health` as readiness | Health route code/test; PR12 Game Day A simulation |
| PayPal timeout/provider failure | 502/504; `paypal.client.*`; `timeout`/`provider_error` span outcome | Current provider operation fails or remains ambiguous | Local transaction does not claim trusted completion | Read-only GET may retry; later webhook/reconciliation retries safe reads | Check mode/credentials/provider status and durable ids; do not blindly retry mutating calls | PayPal failure-policy tests; PR12 Game Day B |
| PayPal success but response lost | Local row remains unresolved; later provider GET/webhook shows success | Client sees failure/timeout despite remote effect | Payment/refund claim and stable identities remain where already committed | Capture: webhook/GET reconciliation. Refund: stable request-id replay/GET. OrdersCreate remains AUD-020 limitation | Preserve ids; reconcile provider evidence before any new mutation | Refund integration tests; AUD-020 explicitly frozen |
| Duplicate webhook | duplicate log/counter; unique provider event identity | Repeat delivery returns idempotent outcome | One event identity and no duplicate economic effect | Unique constraint plus confirmation claim | No action unless duplicate rate is abnormal | PostgreSQL webhook suite; PR12 Game Day C |
| Invalid webhook | verification failure, 400, failed counter/span | Event is rejected before trusted state change | No trusted payment transition | Provider can redeliver a valid event | Check webhook id, clock skew, signature headers, cert host; never bypass verification | Webhook verification/security tests |
| Reservation expires during payment | `reservation_expired` outcome; order/listing/refund queries | Listing is not sold after its hold; captured funds cannot fulfill | Order stays unfulfilled; one refund obligation when capture is trusted | Expiry and payment guards; refund reconciliation | Follow order/refund investigation; do not force `SOLD` | Reservation/payment race tests and webhook integration |
| Refund pending | Refund status/age query; pending/retry metric/log | Buyer funds remain under compensation workflow | `Refund` PENDING/PROCESSING with stable capture/request identity | Bounded sweep rechecks/replays safely | Wait inside policy; investigate at age threshold | Refund integration suite; PR12 Game Day D |
| Refund failed | terminal counter/log and `Refund.failureReason` | Automatic path cannot claim completion | FAILED row and provider identity retained | Known provider id is re-read; later COMPLETED may win | Inspect exact PayPal refund; escalate; never issue a second refund | Refund integration suite |
| Remote refund success/local failure | Provider says COMPLETED while local refund remains PROCESSING/FAILED | Local order/ledger temporarily stale | Durable obligation remains; request identity is stable | Sweep re-observes or safely replays, then commits local completion atomically | Restore normal sweep/DB health; do not edit completion manually | Refund integration suite; PR12 Game Day D |
| Reconciliation interrupted | sweep span/log stops; eligible rows remain old | Batch progress stops | Orders/refunds and timestamps remain in PostgreSQL | Next process/sweep reselects eligible work; conditional claims avoid overlap | Restart/restore service; query oldest eligible work | Payment/refund reconciliation tests |
| Outbox processing interrupted | outbox span/log; stale PROCESSING query | Publication delayed; handler may have run before final mark | PENDING/stale PROCESSING row remains | Next process reclaims stale claim; published transition is idempotent | Restore dispatcher; inspect attempts and terminal failures | PostgreSQL outbox suite; PR12 Game Day E |
| Email delivery failure | bounded retry log/error from Resend path | Verification message may be delayed or duplicated | Same verification code remains valid for the workflow | Retry policy for transient read/network/5xx/429 class | Verify provider/config; ask user to retry; do not expose code in production logs | Failure-recovery tests and external-HTTP policy |
| App restart/deploy drain | shutdown logs; `/ready` 503 while `/health` remains 200 | New traffic drains; in-flight work has a 10s window | PostgreSQL transactions, claims, and outbox/refund rows survive | Jobs restart; stale durable work is reclaimed | Confirm new `/ready`; correlate dropped requests; inspect pending work | Lifecycle tests and durable-work tests |

Evidence labels above name the closest source. The PR12 point-in-time artifact records exact executions. Production OTLP, an actual DB outage, a live rollback, and a live restore were not exercised.
