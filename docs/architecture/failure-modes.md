...
|---|---|---|---|---|---|---|
| PostgreSQL unavailable | `/ready` 503; Prisma error/log; `db.client.errors` if OTLP exports | Instance is not ready; DB-backed requests fail | Previously committed rows unchanged | Render stops routing to an unready replacement; app recovers when DB returns | Check Render DB/service health and connection configuration; do not use `/health` as readiness | Health route code/test; PR12 Game Day A simulation |
